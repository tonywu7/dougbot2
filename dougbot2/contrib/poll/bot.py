...
class Poll:
    ...
    RE_EDIT_TIME = re.compile(
        (r"^<@(?P<user_id>\d+)> <t:(?P<modified>\d+).*>$"), re.MULTILINE
    )
    RE_COMMENT = re.compile(
        (r"^<@(?P<user_id>\d+)> <t:(?P<created>\d+).*>:" r" (?P<content>.+)$"),
        re.MULTILINE,
    )
    ...
    def __init__(
        self,
        content: str,
        choices: dict[EmoteType, Optional[str]],
        *,
        author: str = "unknown",
        title: str = "Poll",
    ):
    ...
    def load_votes(self, text: str):
        ...
        self.votes = [Vote(**v.groupdict()) for v in self.RE_VOTE.finditer(text)]

    def load_comments(self, text: str):
        """Parse added comments from text (the "Comments" field) and add them to this poll."""
        text = self.deobfuscate(text)
        self.comments = [
            Comment(**c.groupdict()) for c in self.RE_COMMENT.finditer(text)
        ]
    ...
    def load_edits(self, text: str):
        ...
        text = self.deobfuscate(text)
        self.edits = [
            EditTime(**e.groupdict()) for e in self.RE_EDIT_TIME.finditer(text)
        ]
    ...
    def tally(self, origin: Message, hide_username: bool = False) -> Embed2:
        ...
        votes = map_reduce(
            self.votes,
            lambda v: f"{v.emote} {strong(v.text)}",
...
class Polling(
    ...
    async def list_suggest_channels(self, guild: Guild) -> str:
        ...
        q = SuggestionChannel.objects.filter(snowflake__in=channels)
        lines = []
        targets = sorted(
            ((guild.get_channel(c.snowflake), c) for c in await async_list(q)),
    ...
    async def send_channel_list(self, ctx: Surroundings):
        ...
        pages = [
            base.set_description(description=description)
    ...
    async def get_channel_or_404(self, ctx: Surroundings, channel: TextChannel):
        ...
        suggests = SuggestionChannel.objects.filter(snowflake__in=channels)
        try:
            suggest: SuggestionChannel = await async_get(suggests, snowflake=channel.id)
        ...
        *,
        ...
        if linked:
            ...
            with suppress(HTTPException):
                await linked_msg.edit(
                    allowed_mentions=AllowedMentions.none(), content=linked
                )
    ...
    async def get_suggestion_files(
        self, target: SuggestionChannel, msg: Message
    ) -> list[File]:
    ...
    @doc.argument(
        "category",
        "The suggestion channel to use.",
        node="[suggest channel]",
        signature="[channel]",
    )
    ...
    async def suggest(
        ...
        *,
        ...
        if not category.permissions_for(ctx.author).read_messages:
            raise NotAcceptable(
                (
                    f"You cannot submit to {tag(category)} because"
    ...
    @doc.restriction(
        None, "You can only change the attribution of a suggestion you submitted."
    )
    @doc.cooldown(1, 5, BucketType.member)
    @doc.hidden
    @on_error_reset_cooldown
    async def suggest_credit(
        ...
        if not poll.can_delete(ctx.author):
            raise NotAcceptable(
                "You can only change the attribution of a suggestion you submitted."
            )
    ...
    @doc.restriction(
        None, "You can only change the comment access of a suggestion you submitted."
    )
    @doc.cooldown(1, 5, BucketType.member)
    @doc.hidden
    async def suggest_forum(
        ...
        if not poll.can_delete(ctx.author):
            raise NotAcceptable(
                "You can only change the comment access of a suggestion you submitted."
            )
        poll.forum = enabled
        await self.update_submission(poll, suggestion)
        res = (
            f"Comments section for suggestion {code(suggestion.id)} is now"
    ...
    async def poll(self, ctx: Surroundings, *, content: str = ""):
        ...
        emotes = list(
            map(
                ...
                " :keycap_6: :keycap_7: :keycap_8: :keycap_9: :keycap_10:".split(" "),
            )
        )
        lines: dict[str, str] = {}
    ...
    @doc.argument(
        "anonymize", "Whether to omit vote casters' username from the result."
    )
    @doc.hidden
    @can_embed
    ...
    async def on_bulk_delete(self, ev: RawBulkMessageDeleteEvent):
        ...
        await asyncio.gather(
            *[self.delete_linked_msg(id_, channel) for id_ in ev.message_ids]
        )
