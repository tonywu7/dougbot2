...
from dougbot2.utils.common import Embed2, a, can_upload, code, strong, trunc_for_field
...
class MessageCommands:
    ...
    @doc.argument(
        "content", "The text message to send.", node="content", signature="content"
    )
    @doc.argument(
        "embed",
        "The embed to send.",
    ...
    @doc.argument(
        "mentions", ("The members/roles this message" " is allowed to notify.")
    )
    ...
    async def redirect(
        ...
        *,
        ...
        embed = first(message.embeds, None)
        files = [
            await att.to_file(spoiler=att.is_spoiler()) for att in message.attachments
        ]
        if isinstance(mentions, dict):
            allowed_mentions = get_allowed_mentions(mentions)
        else:
            allowed_mentions = None
        msg = await channel.send(
            content=content, embed=embed, files=files, allowed_mentions=allowed_mentions
        )
        url = a("Message created:", msg.jump_url)
        reply = Embed2(description=f"{url} {code(msg.id)}")
        await ctx.respond(embed=reply).reply().run()
    ...
    async def react(
        ...
        if failed:
            ...
            res = Embed2(
                description=f'{strong("Failed to add the following emotes")}\n{failed_list}'
            )
