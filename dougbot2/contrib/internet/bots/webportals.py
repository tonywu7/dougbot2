...
class WebPortalCommands:
    ...
    @doc.invocation((), 'Get a sequence by searching for a random A-number.')
    ...
    async def oeis(
        ...
        async with ctx.typing():
            try:
                oeis = OEIS(ctx.bot.get_web_client())
                if query:
                ...
                await ctx.respond(reason).autodelete(20).run()
            except aiohttp.ClientError:
                await ctx.respond('Network error while searching on OEIS').run()
                raise

        async def more_result(*args, **kwargs):
            await ctx.respond(f'({num_results - 1} more {english.pluralize(num_results - 1, "result")})').run()

        (
            await ctx.respond(embed=sequence.to_embed())
            .callback(more_result).deleter()
            .run(thread=True)
        )
