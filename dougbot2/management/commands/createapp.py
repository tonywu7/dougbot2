...
class Command(StartAppCommand):
    ...
    def add_arguments(self, parser) -> None:
        ...
        parser.add_argument(
            ...
            dest="cog_sort_order",
            default=50,
            help="Sort order of the new Cog to be used in the help command.",
        )

    def handle(self, **options):
        options["template"] = str(
            (Path(__file__).with_name("templates") / "app.tar.gz").resolve(strict=True)
        )
        ...
        logger.info(
            (
                f"\nName: {qual_name}"
                f"\nPath: {target.resolve()}"
                f'\n{colored("Remember to add %s to INSTALLED_APPS" % (qual_name,), "blue", attrs=["bold"])}'
            )
        )
