...
from ..converters import (
    InvalidChoices, NumberOutOfBound, RegExpMismatch, datetime, geography,
)
...
def setup(bot: MissionControl):
    ...
    doc.register_type(converters.Timedelta, NP('duration', predicative='such as `60s`, `1h13m36s`, or `3w6d`'))
    ...
    doc.register_type(converters.Lowercase, NP('text', predicative='case insensitive', uncountable=True))
    ...
    doc.register_type(
        ...
        NP(
            'IANA tz code',
            predicative=('see [list of timezones](https://en.wikipedia.org/' 'wiki/List_of_tz_database_time_zones)'),
        ),
    )
    doc.register_type(geography.Latitude, NP('latitude', predicative='such as `-41.5`, `41.5N`, or `N 39°`'))
    doc.register_type(geography.Longitude, NP('longitude', predicative='such as `-110`, `30E`, or `E 162°`'))
